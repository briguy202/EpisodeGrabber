...
namespace EpisodeGrabber {
	...
	public partial class MainWindow : Window {
		...
		public static RoutedUICommand RefreshSelectedCommand = new RoutedUICommand("Refreshes selected tree node", "RefreshSelectedCommand", typeof(MainWindow));
		...
		public static RoutedUICommand ClearGrabberFiles = new RoutedUICommand("ClearGrabberFiles", "ClearGrabberFiles", typeof(MainWindow));
		...
		private bool IsEntitySelected() {
			return this.GetSelectedTreeItemTag() is EntityBase;
		}

		private bool IsShowSelected() {
			return this.GetSelectedTreeItemTag() is Show;
		}

		private bool IsEpisodeSelected() {
			return this.GetSelectedTreeItemTag() is Episode;
		}
		...
		private List<string> GetExpandedNodes(TreeViewItem node) {
			...
			foreach (TreeViewItem childNode in node.Items) {
				if (childNode.IsExpanded && childNode.Tag is EntityBase) {
					expanded.Add(((EntityBase)childNode.Tag).UniqueID);
			...
			if (configuration.ScanFolders != null && configuration.ScanFolders.Count > 0) {
				foreach (string folder in configuration.ScanFolders) {
					...
					if (directory.Exists) {
						...
						try {
							entities = EntityServiceBase.ScanDirectory(directory.FullName, configuration.MediaTypes);
						} catch (Exception ex) {
		...
		private void RefreshNode(TreeViewItem node) {
			ValidationUtility.ThrowIfNullOrEmpty(node, "node");
		...
		private void AddNode(TreeViewItem parent, TreeViewItem node, List<string> expandedIDs) {
			this.AddNode(parent, node, expandedIDs, -1);
		}
		...
		private TreeViewItem GetSelectedTreeItem() {
			if (this.treeView1 != null && this.treeView1.SelectedItem != null) {
				return (TreeViewItem)this.treeView1.SelectedItem;
			}
			return null;
		}

		private object GetSelectedTreeItemTag() {
		...
		private void FetchMetaData(EntityBase entity) {
			ValidationUtility.ThrowIfNullOrEmpty(entity, "entity");

			_isBusy = true;
			var fetchService = EntityServiceBase.GetService(entity) as IFetchService;
			TraceManager.Trace(string.Format("Fetching metadata for '{0}'.", entity.Name), TraceVerbosity.Minimal, TraceTypes.OperationStarted);
			...
			worker.DoWork += (sender, args) => {
				...
				EntityBase entityParam = (EntityBase)arguments["entity"];

				if (entityParam.ID == 0) {
					// The entity exists in the user's folder, but hasn't been fetched yet, so go search for the it.
					TraceManager.TraceFormat("Searching by name for '{0}'.", entityParam.Name);
					foundEntities = fetchService.FetchMetadataByName(entityParam.Name);
				} else {
				...
				if (foundEntities.Errors.IsNullOrEmpty()) {
					if (foundEntities.Data.IsNullOrEmpty()) {
						...
						if (foundEntities.Data.Count > 1) {
							ChooseShow window = new ChooseShow(entityParam.Name, foundEntities.Data);
						...
						EntityBase selectedEntity = (EntityBase)foundEntities.Data[0];
						...
						workerByID.DoWork += (sender2, args2) => {
							...
							EntityBase finalEntity = fetchService.FetchMetadataByID(id).Data;
							if (finalEntity.ID != 0) {
								var service = EntityServiceBase.GetService(finalEntity) as IEntityService;
								finalEntity.Path = oldFolderPath;
								service.Save(finalEntity);
								service.CreateFiles(finalEntity, true, _configuration);	// Get all banners, metadata files, etc.
							}
						};
		...
		private void FetchMetaData(IEnumerable<EntityBase> entities) {
			foreach (EntityBase entity in entities) {
			...
			if (!entities.IsNullOrEmpty() && !_isBusy) {
				...
				worker.DoWork += (sender, args) => {
					...
					foreach (EntityBase entity in entitiesAsync) {
						EntityServiceBase.GetService(entity).Delete(entity);
		...
		private void MoveFilesAsync(List<Show> allShows) {
			...
			worker.DoWork += (sender, args) => {
				...
				List<FileInfo> files = directory.GetFiles("*.*").Where((f) => _configuration.MediaTypes.Contains(f.Extension.TrimStart('.'))).ToList();
				bool foundShowToMove = false;

				TraceManager.TraceFormat("Found {0} files that end in supported extensions of {1}.", files.Count.ToString(), string.Join(", ", _configuration.MediaTypes));
				//TraceManager.TraceFormat("All files: {0}.", string.Join(", ", directory.GetFiles("*.*").Select((fi) => fi.Extension.TrimStart('.'))));
						...
						if (match.Success) {
							...
							if (season != null) {
								if (string.IsNullOrEmpty(season.Path)) {
									string createDirectoryPath = System.IO.Path.Combine(show.Path, season.Name);
								...
								showService.Initialize(show, _configuration.MediaTypes);
								showService.CreateFiles(show, true, _configuration);
							} else {
		...
		private void ClearGrabberFilesExecute() {
			foreach (string folder in UserConfiguration.GetConfiguration().ScanFolders) {
				EntityServiceBase.DeleteCache(folder);
			}
			this.Refresh();
		}
		...
		private void FetchMetadata_Execute(object sender, ExecutedRoutedEventArgs e) {
			...
			if (entity != null) {
				this.FetchMetaData((EntityBase)entity);
			} else if (this.IsRootNodeSelected()) {
				this.FetchMetaData(this.GetChildEntities().OfType<EntityBase>());
			}
		}
		...
		private void FetchMetadata_CanExecute(object sender, CanExecuteRoutedEventArgs e) {
			var entity = this.GetSelectedTreeItemTag() as EntityBase;
			if (entity != null) {
				var service = EntityServiceBase.GetService(entity);
				var serviceName = (service != null) ? service.GetType().Name : "null service";
				//TraceManager.Trace(string.Format("CanExecute - Busy:{0}, IsIFetchService:{1}, Service:{2}", _isBusy, (service is IFetchService), serviceName), TraceVerbosity.Verbose);
				e.CanExecute = this.treeView1 != null && !_isBusy && (service is IFetchService || this.IsRootNodeSelected() || this.IsMissingShowSelected());
			}
		}
		...
		private void RefreshSelected_Execute(object sender, ExecutedRoutedEventArgs e) {
			this.RefreshNode(this.GetSelectedTreeItem());
		}
		...
		private void Download_Execute(object sender, ExecutedRoutedEventArgs e) {
			Episode episode = this.GetSelectedTreeItemTag() as Episode;
			if (episode != null) {
				...
				downloadString = downloadString.Replace(",", "%2C");
				...
				try {
					Process.Start(downloadString);
				} catch (Exception ex) {
					MessageBox.Show(string.Concat("Error while attempting to open location '", downloadString, "': ", ex.Message), "Error", MessageBoxButton.OK, MessageBoxImage.Error);
				}
			}
		}
		...
		private void ShowEntitySelected_CanExecute(object sender, CanExecuteRoutedEventArgs e) {
			e.CanExecute = this.GetSelectedTreeItemTag() is EntityBase;
		}
		...
		private void ClearGrabberFiles_Execute(object sender, ExecutedRoutedEventArgs e) {
			this.ClearGrabberFilesExecute();
		}
		...
		private void CreateFiles_Execute(object sender, ExecutedRoutedEventArgs e) {
			...
			EntityServiceBase.GetService(entity).CreateFiles(entity, true, _configuration);
		}
		#endregion
